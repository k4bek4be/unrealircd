...
#define HOOKTYPE_POST_LOCAL_QUIT 108
...
int hooktype_post_local_quit(Client *client, MessageTag *mtags, char *comment);
...
#define ValidateHooks(hooktype, func) \
    if (((hooktype == HOOKTYPE_LOCAL_QUIT) && !ValidateHook(hooktype_local_quit, func)) || \
        ...
        ((hooktype == HOOKTYPE_POST_LOCAL_QUIT) && !ValidateHook(hooktype_post_local_quit, func)) ) \
